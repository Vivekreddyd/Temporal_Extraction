...
for root, dirs, files in os.walk('/home/vivek/tweepy_dataset/Text_only'):
    for file in files:
        with open(os.path.join(root,'travel_new8.txt'),"r") as tweets_file:
            ...
            prev_line=None
            #print file
            tweets_file1=sorted(tweets_file)
            #for item in tweets_file1:
                #print>>tweets_new_file, item
            print tweets_file1[:50]
            ...
            # The checking condition is over here is because first it needs to convert https:.. into URL to compare otherwise the results are not good
                if prev_line is None or not clean_line1.startswith(prev_line):
                    ...
                    length=len(clean_line2.split())
                    if (len(clean_line2.split())<4):
                        continue
                    prev_line = clean_line1
                    #print clean_line2
                    reload(sys)
                    sys.setdefaultencoding('utf8')
